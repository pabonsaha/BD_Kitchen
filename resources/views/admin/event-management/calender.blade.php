...
<script src="{{ asset('assets/vendor/libs/fullcalendar/fullcalendar.js') }}"></script>
    <script>
        $( document ).ready(function() {
            $.ajax({
                url: '/event-management/event/calender-events',
                type: 'GET',
                success: function(response) {
                    ...
                    allEvents.forEach(event => {
                        // You can customize the event properties as per your requirement
                        events.push({
                            id: event.id,
                            url: event.url || '', // Fallback to empty string if no URL is provided
                            title: event.title || 'No Title', // Fallback to 'No Title' if title is missing
                            start: event.start, // Assuming the event object has a start date
                            end: event.end, // Assuming the event object has an end date
                            allDay: false, // Fallback to false if allDay is not specified
                            extendedProps: {
                                calendar: event.type || 'General' // Fallback to 'General' if calendar is not specified
                            }
                        });
                    });
                    let calendarEl = document.getElementById('calendar'); // Ensure you have an element with this ID

                    let calendar = new Calendar(calendarEl, {
                        plugins: [dayGridPlugin, interactionPlugin, listPlugin, timegridPlugin],
                        events: events // Load the processed events into the calendar
                    });
