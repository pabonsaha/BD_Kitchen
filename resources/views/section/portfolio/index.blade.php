...
@section('title', $title ?? _trans('portfolio.Portfolio'))
...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        {!! breadcrumb(_trans('portfolio.Portfolio').' '._trans('order.List'),['section/portfolioAndInspiration/1/index'=>_trans('portfolio.Portfolio').' & '._trans('portfolio.Inspiration'),'portfolio'=>_trans('portfolio.Portfolio').' '._trans('order.List')]) !!}
        ...
        <div class="app-ecommerce-category">
            ...
            <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEBrandList"
                ...
                <div class="offcanvas-header py-4">
                    <h5 id="offcanvasEcommerceCategoryListLabel" class="offcanvas-title">{{_trans('common.Add').' '._trans('portfolio.Portfolio')}}</h5>
                ...
                <div class="offcanvas-body border-top">
                    <form class="pt-0" id="addModal" method="POST">
                        <!-- Title -->
                        <div class="mb-3">
                            <label class="form-label" for="ecommerce-category-title">{{_trans('common.Title')}}</label>
                            <input type="text" class="form-control" id="ecommerce-category-title"
                                placeholder="Enter portfolio name" name="name" aria-label="Brand Name" />
                            <span class="text-danger nameError error"></span>
                        </div>
                        <!-- Image -->
                        <div class="mb-3">
                            <label class="form-label" for="brand-image">{{_trans('portfolio.Portfolio').' '._trans('common.Image')}}</label>
                            <input class="form-control" type="file" name="image" id="brand-image" />
                            <span class="text-danger imageError error"></span>
                        </div>
                        <!-- Status -->
                        <div class="mb-4 ecommerce-select2-dropdown">
                            <label class="form-label">{{_trans('common.Select').' '._trans('portfolio.Category')}}</label>
                            <select id="category_id" name="category_id" class="select2 form-select"
                                data-placeholder="Select category">
                                <option value="">{{_trans('common.Select').' '._trans('portfolio.Section').' '._trans('portfolio.Category')}}</option>
                        ...
                        <!-- Status -->
                        <div class="mb-4 ecommerce-select2-dropdown">
                            <label class="form-label">{{_trans('common.Select').' '._trans('portfolio.Portfolio').' '._trans('common.Status')}}</label>
            ...
            <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasBrandEditModal"
                ...
                <!-- Offcanvas Header -->
                <div class="offcanvas-header py-4">
                    <h5 id="offcanvasEcommerceCategoryListLabel" class="offcanvas-title">{{_trans('common.Edit').' '._trans('portfolio.Portfolio')}}</h5>
                    <button type="button" class="btn-close bg-label-secondary text-reset" data-bs-dismiss="offcanvas"
                        aria-label="Close"></button>
                </div>
                <!-- Offcanvas Body -->
                <div class="offcanvas-body border-top">
                    <form class="pt-0" id="updateCategoryModal" method="POST">
                        <!-- Title -->
                        <div class="mb-3">
                            <label class="form-label" for="edit_name">Title</label>
                        ...
                        <div class="mb-4 ecommerce-select2-dropdown">
                            <label class="form-label">{{_trans('common.Select').' '._trans('portfolio.Section').' '._trans('portfolio.Category')}}</label>
    ...
    <script>
        $(function() {
            ...
            var table = $('.data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url : '{{ route('section.portfolioAndInspiration.index', 1) }}',
                    data: function (d){
                        d.status = $('#status').val()
                    }
                },
                ...
                buttons: [
                    ...
                    {
                        text: '<i class="ti ti-plus ti-xs me-0 me-sm-2"></i><span class="d-none d-sm-inline-block">Add Portfolio</span>',
            ...
            $('#addModal').on('submit', function(e) {
                ...
                $('.error').text('');
                $.ajax({
                    url: '{{ route('section.portfolioAndInspiration.store', 1) }}',
                    type: 'POST',
            ...
            $(document).on("click", ".brand_edit_button", function() {
                ...
                $.ajax({
                    url: '/section/portfolioAndInspiration/1/' + $id,
                    type: 'GET',
                    success: function(response) {
                        ...
                        $("#edit_category_id").find('option').removeAttr("selected");
                        $('#edit_category_id').trigger('change.select2');
                        $('#edit_category_id').find('option[value="' + response.data
            ...
            $('#updateCategoryModal').on('submit', function(e) {
                ...
                $('.error').text('');
                $.ajax({
                    url: '{{ route('section.portfolioAndInspiration.update', 1) }}',
                    type: 'POST',
                    contentType: 'multipart/form-data',
                    cache: false,
                    contentType: false,
            ...
            $(document).on("click", ".brand_delete_button", function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: '{{ route('section.portfolioAndInspiration.destroy', 1) }}',
                            method: 'POST',
                            data: {
                                "_token": "{{ csrf_token() }}",
                                section_id: id,
