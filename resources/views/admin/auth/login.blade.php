                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        @if (env('APP_ENV') == 'local' && env('APP_DEBUG') == 'true')
                            @php
                                $users = \App\Models\User::select(['email','role_id'])->with('role')
                                    ->whereIn('role_id', [
                                        \App\Models\Role::ADMIN,
                                        \App\Models\Role::KITCHEN,
                                    ])->get()->groupBy('role_id');
                                    $key = 0;
                            @endphp
                            <div class="d-flex">
                                @foreach ($users as $user)
                                    <form method="POST" class="loginForm" action="<?php echo e(route('admin.login')); ?>">
                                        @csrf
                                        <input type="hidden" name="email" value="{{ $user[$key]->email }}">
                                        <input type="hidden" name="password" value="password">
                                        <button type="submit" class="btn btn-rounded btn-info mt-2 mb-2 mx-2">{{$user[$key]->role->name}}</button>
                                    </form>
                                @endforeach
                            </div>
                        @endif
